...
-- get country coude:
SELECT code, name FROM country WHERE name = 'China'; --code is CHN
...
-- this is the same with an IN comparison
...
-- The name, population, and GNP of all countries with a GNP greater than $1 trillion dollars and a population of less than 1 billion peopl
...
SELECT name, lifeexpectancy, region FROM country WHERE region LIKE  'South%' AND lifeexpectancy IS NOT NULL;
...
SELECT name, continent, gnp, lifeexpectancy FROM country WHERE (continent = 'Africa' OR continent = 'Asia') AND lifeexpectancy >= 70 AND (gnp BETWEEN 1000000 AND 100000000);
