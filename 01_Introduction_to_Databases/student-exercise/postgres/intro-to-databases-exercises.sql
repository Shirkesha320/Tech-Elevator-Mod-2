...
Select name, population FROM city WHERE district In ('Ontario');

Select * from city
-- 2. The name and population of all cities in Montana (1 row)
SELECT name, population FROM city WHERE district IN ('Montana');
...
select *from country;
SELECT name continent, gnp, lifeexpectancy FROM country WHERE (lifeexpectancy > 70) and (gnp >= 1) and (gnp <= 110);

-- 21. The per capita GNP (i.e. GNP divided by population) in US Dollars of all countries in Europe (46 rows)
select *from country;
SELECT (gnp / population) as PerCapitalGNP FROM country WHERE continent = 'Europe';

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
select *from country;
SELECT (2021 - indepyear) as YYearsSinceIndependence FROM country WHERE indepyear IS NOT NULL;
